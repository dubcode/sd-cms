...
- [Nuxt Build Procedure](#nuxt-build-procedure)
  - [Nuxt Config](#nuxt-build-procedure)
    - [Global Meta Tags](#meta-tags--open-graph)
...
Yoast SEO
[https://en-gb.wordpress.org/plugins/wordpress-seo/](https://en-gb.wordpress.org/plugins/wordpress-seo/)
...
Open up nuxt.config.js and add the following to add global meta and open graph tags for SEO. These tags will be used as a fallback when no other meta data os available on pages and posts etc.
...
We have already added global meta tags to our nuxt.config.js, these will work as a fallback for when no other meta tags have been set. Now we need to add page / post level meta tags into our templates. A simple setup would be to simply hardcode them in much like we did in the nuxt.config.js
...
But in this case we can grab the meta data from Yoast via the WordPress API as follows.
